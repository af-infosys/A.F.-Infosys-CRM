...
const SurvayorReport = () => {
  ...
  useEffect(() => {
    generateReport();
  }, [selectedDate, allCalls, surveyors]);

  const generateReport = () => {
    if (surveyors.length === 0 || allCalls.length === 0) {
      setReportData([]);
      return;
    }

    // Create a map to store call counts for each telecaller.
    const callCounts = surveyors.reduce((acc, telecaller) => {
    ...
    const finalReport = surveyors.map((surveyor) => ({
      ...surveyor,
      callCount: callCounts[surveyor._id] || 0,
    }));
  ...
  const getTelecallerName = (id) => {
    const caller = surveyors.find((t) => t._id === id);
                    ...
                    {reportData.length > 0 ? (
                      reportData.map((surveyor) => (
                        ...
                        >
                          ...
                          <td className="py-3 px-4">
                            <button
                              onClick={() => viewDetails(surveyor._id)}
